...
export const HematologyCategoryPage: React.FC = () => {
  const [activeFilter, setActiveFilter] = useState<'Protein Analyzer'>('Protein Analyzer');
  ...
  const products: Product[] = [
  {
    ...
    "brand": "Agappe",
    "type": "Protein Analyzer",
    "imageKey": "product-67.png",
    "documentKey": "Agappe/Mispa-i2.pdf",
    "features": [
      "Rate based Nephelometry & Immuno-turbidimetry method.",
      "UCS technology – sensitivity & linearity at its best.",
      "Smart card calibration.",
      "3 step user friendly procedure.",
      "Auto incubation, Auto rotation, Auto mixer.",
      "500 patient results storage.",
      "25 parameter assay model.",
      "Patient ID entry facility along with Barcode reader.",
      "LIS compatibility."
    ]
  },
  {
    ...
    "brand": "Agappe",
    "type": "Protein Analyzer",
    "imageKey": "product-68.png",
    "documentKey": "Agappe/Mispa-i3.pdf",
    "features": [
      "Rate based Nephelometry & Immuno-turbidimetry method.",
      "Patented Unique Channel Shifting (UCS) Technology – Sensitivity and Linearity at its best.",
      "Prozone detection function, No false reporting due to Prozone detection feature.",
      "Smart card calibration.",
      "Zero calibration cost due to Smart card Calibration.",
      "Prefilled cartridge with reagents.",
  ...
  },
  {
    ...
    "type": "Protein Analyzer",
    "imageKey": "product-69.png",
    "documentKey": "Labcare_Diagnostics/Accuturb-100.pdf",
    "features": [
      "Fast and quantitative tests for specific protein controlled by microcomputer.",
      "Test result can be reported within 2 minutes.",
      "Dual system of nephelometry and turbidimetric tests, high sensitivity, wide testing range.",
      "Specimen dilution and calibration of hematocrit (HCT) make the result uniform.",
      "Single channel or dual channel, tests specimens independently in different programs.",
      "Automatic Point Of Care Testing.",
      "RF non contact card with saved reagents paraments.",
      "Shows all the results on the screen, easy to print."
    ]
  },
  {
    "id": 4,
    ...
    "brand": "Tulip",
    "type": "Protein Analyzer",
    "imageKey": "product-70.png",
    "documentKey": "Tulip/Turbodyne-SC.pdf",
    "features": [
      "Dual technology optics secures turbidimetric/nephelometric measurements selectively.",
      ...
      "Disposable cuvette secures no carryover effects, reduces labour and minimizes biohazard."
    ]
  }
];

  const filteredProducts = products.filter(p => activeFilter === 'Protein Analyzer' || p.type === activeFilter);
      ...
      <section className="pt-16 pb-6 container mx-auto px-4 max-w-[1350px]">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-slate-200/60 pb-6">
          ...
          <div className="flex items-center gap-2 bg-slate-200/60 p-1 rounded-xl border border-slate-200">
            {(['Protein Analyzer'] as const).map((filter) => (
              <button
                key={filter}
              ...
              >
                {filter === 'Protein Analyzer' ? 'View All Models' : `${filter} Range`}
              </button>
            ))}
          </div>
        </div>
