...
interface Product {
  ...
  name: string;
  brand: string;
  type: 'Elisa Reader' | 'Elisa Washer';
  imageKey: string;
  documentKey: string;
  features: string[];
}
...
export const HematologyCategoryPage: React.FC = () => {
  const [activeFilter, setActiveFilter] = useState<'All' | 'Elisa Reader' | 'Elisa Washer'>('All');
  ...
  const products: Product[] = [
  {
    ...
    "brand": "J-Mitra",
    "type": "Elisa Reader",
    "imageKey": "product-51.png",
    "documentKey": "J-Mitra/ER-181s-Reader.pdf",
    "features": [
      "User friendly windows operation system with touch screen and large LCD display.",
      "Storage capacity: 100 test protocols and 10000 sample records.",
      "Multiform result output including patient comprehensive report.",
      "Bichromatic measurement including 4 PL methods for quantitative tests.",
      "Auto self check, lamp sleeping and wake up function.",
      "8 channel optical fiber system.",
  ...
  },
  {
    ...
    "brand": "J-Mitra",
    "type": "Elisa Washer",
    "imageKey": "product-52.png",
    "documentKey": "J-Mitra/ER-181s-Washer.pdf",
    "features": [
      "User friendly windows operation system with touch screen and large LCD display.",
      "Large memory, up to 100 wash protocols can be programmed.",
      "Liquid level detection and alert function.",
      "Multi channel, 3 bottles: 1 each for wash, rinse & waste.",
      "Built in incubator with user adjustable temperature settings at 25°C, 30°C & 37°C.",
      "Plate shaking function, time and speed are adjustable.",
      "Washes flat, U and v bottom strips and plates.",
      "12 ways or 8 ways manifold, automatic & manual positioning."
    ]
  }
];
      ...
      <section className="pt-16 pb-6 container mx-auto px-4 max-w-[1350px]">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-slate-200/60 pb-6">
          ...
          <div className="flex items-center gap-2 bg-slate-200/60 p-1 rounded-xl border border-slate-200">
            {(['All', 'Elisa Reader', 'Elisa Washer'] as const).map((filter) => (
              <button
                key={filter}
